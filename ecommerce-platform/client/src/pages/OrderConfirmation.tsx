...
export default function OrderConfirmation() {
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="container py-20 text-center flex-1">
          ...
          <Link href="/account/orders">
            <Button>{t("orderConfirmation.viewMyOrders")}</Button>
          </Link>
        </div>
        <Footer />
      </div>
  ...
  const paymentAccount =
    order.paymentMethod === "paypal" ? PAYPAL_ACCOUNT : ALIPAY_ACCOUNT;
      ...
      <div className="container py-8 flex-1 max-w-3xl mx-auto">
        ...
        <Card className="border-gold/30 bg-gold/5 mb-6">
          <CardContent className="p-6">
            <h3 className="text-lg font-semibold text-foreground mb-3">
              {t("orderConfirmation.paymentInstructions")}
            </h3>
            <p className="text-sm text-muted-foreground mb-4">
              {t("orderConfirmation.pleaseCompletePayment")}{" "}
              {order.paymentMethod === "paypal" ? "PayPal" : "Alipay"}{" "}
              {t("orderConfirmation.toFollowingAccount")}
            </p>
            <div className="flex items-center gap-2 bg-background p-3 rounded-lg border">
              ...
              <button
                onClick={() => {
                  ...
                  toast.success(t("orderConfirmation.copied"));
                }}
            ...
            <p className="text-xs text-muted-foreground mt-3">
              {t("orderConfirmation.includeOrderNumber")}{" "}
              <span className="font-semibold text-foreground">
                {order.orderNumber}
              </span>{" "}
              {t("orderConfirmation.paymentReference")}
            </p>
          </CardContent>
        </Card>
        ...
        <div className="bg-muted/30 rounded-lg p-4 mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {t("orderConfirmation.phone")} {order.shippingPhone}
          </p>
        </div>
