...
import { FileText, Trash2, Minus, Plus, Send, ArrowLeft, CheckCircle2 } from "lucide-react";
...
export default function InquiryList() {
  ...
  const [submitted, setSubmitted] = useState<{ inquiryNumber: string } | null>(null);
  ...
  const inquiryQuery = trpc.inquiry.list.useQuery(undefined, { enabled: isAuthenticated });
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <div className="container py-20 text-center">
          ...
          <h2 className="text-xl font-semibold mb-2">{t("inquiry.signInRequired")}</h2>
          <p className="text-muted-foreground mb-6">{t("inquiry.signInToView")}</p>
          <a href={getLoginUrl()}>
            <Button className="bg-gold text-charcoal-dark hover:bg-gold-dark">{t("inquiry.signIn")}</Button>
          </a>
        </div>
        <Footer />
      </div>
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <div className="container py-20 text-center max-w-lg mx-auto">
          ...
          <p className="text-xl font-semibold text-gold-dark mb-6">{submitted.inquiryNumber}</p>
          ...
          <div className="flex gap-3 justify-center">
            ...
            <Link href="/account">
              <Button className="bg-gold text-charcoal-dark hover:bg-gold-dark">{t("inquiry.myAccount")}</Button>
            </Link>
          </div>
        </div>
        <Footer />
      ...
      <div className="container py-8 flex-1">
        ...
        {items.length === 0 ? (
          <div className="text-center py-16 border rounded-lg bg-muted/30">
            <FileText className="h-12 w-12 mx-auto text-muted-foreground/40 mb-4" />
            <h3 className="text-lg font-semibold mb-2">{t("inquiry.listEmpty")}</h3>
            <p className="text-muted-foreground mb-6">
              {t("inquiry.browseAddItems")}
            </p>
            <Link href="/products">
              <Button className="bg-gold text-charcoal-dark hover:bg-gold-dark">{t("inquiry.browseProducts")}</Button>
            </Link>
          </div>
        ) : (
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              {items.map((item: any) => {
                ...
                const image = product.images ? JSON.parse(product.images)[0] : PLACEHOLDER_IMAGE;
                  ...
                  <div key={item.id} className="flex gap-4 p-4 border rounded-lg bg-card hover:shadow-sm transition-shadow">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-muted-foreground mt-0.5">
                        {t("inquiry.unitPrice")} ${Number(product.price).toFixed(2)}
                      </p>
                      <div className="flex items-center gap-3 mt-2">
                        <span className="text-xs text-muted-foreground">{t("inquiry.qty")}</span>
                      ...
                      {item.note && (
                        <p className="text-xs text-muted-foreground mt-1 italic">{t("inquiry.note")} {item.note}</p>
                      )}
                    </div>
                    <button
            ...
            <div className="lg:col-span-1">
              <div className="border rounded-lg p-6 bg-card sticky top-24">
                ...
                <div className="text-sm text-muted-foreground mb-4">
                  <span className="font-medium text-foreground">{items.length}</span> {t("inquiry.productsInInquiry")}
                </div>
                ...
                {!showForm ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-3">
                    <div>
                      <label className="text-xs font-medium text-foreground">{t("inquiry.companyName")}</label>
                      <Input
                    ...
                    <div>
                      <label className="text-xs font-medium text-foreground">
                        {t("inquiry.contactName")} <span className="text-destructive">*</span>
                      </label>
                      <Input
                    ...
                    <div>
                      <label className="text-xs font-medium text-foreground">
                        {t("inquiry.email")} <span className="text-destructive">*</span>
                      </label>
                      <Input
                        type="email"
                    ...
                    <div>
                      <label className="text-xs font-medium text-foreground">{t("inquiry.phone")}</label>
                      <Input
                    ...
                    <div>
                      <label className="text-xs font-medium text-foreground">{t("inquiry.countryRegion")}</label>
                      <Input
                    ...
                    <div>
                      <label className="text-xs font-medium text-foreground">{t("inquiry.additionalMessage")}</label>
                      <Textarea
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      >
                        {submitMutation.isPending ? t("inquiry.submitting") : t("inquiry.sendInquiry")}
                      </Button>
                    </div>
                  </form>
                )}
