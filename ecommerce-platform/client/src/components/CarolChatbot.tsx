...
export default function CarolChatbot() {
  ...
  const WELCOME_MESSAGE: ChatMessage = {
    ...
    options: [
      {
        label: t("carol.requestQuote"),
        value: "quote",
        icon: <ShoppingBag className="h-4 w-4" />,
      },
      {
        label: t("carol.sendInquiry"),
        value: "inquiry",
        icon: <Mail className="h-4 w-4" />,
      },
      {
        label: t("carol.browseProducts"),
        value: "products",
  ...
  const [currentFormType, setCurrentFormType] = useState<
    "product" | "quantity" | "contact" | "details" | null
  >(null);
  ...
  const handleOptionClick = (value: string) => {
    const selectedOption = messages[messages.length - 1]?.options?.find(
      o => o.value === value
    );
    if (selectedOption)
      addMessage({ role: "user", content: selectedOption.label });
      ...
      case "products":
        setTimeout(() => {
          addMessage({
            ...
            options: [
              {
                label: t("carol.goToProducts"),
                value: "go_products",
                icon: <ArrowRight className="h-4 w-4" />,
              },
              {
                label: t("carol.requestQuoteInstead"),
                value: "quote",
                icon: <ShoppingBag className="h-4 w-4" />,
              },
            ],
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (currentFormType === "product") {
      ...
      setTimeout(() => {
        addMessage({
          role: "carol",
          content: t("carol.askQuantity"),
          isForm: true,
          formType: "quantity",
        });
        setCurrentFormType("quantity");
      }, 400);
      ...
      setTimeout(() => {
        addMessage({
          role: "carol",
          content: t("carol.askEmail"),
          isForm: true,
          formType: "contact",
        });
        setCurrentFormType("contact");
      }, 400);
      ...
      setFormData(prev => {
        ...
        setTimeout(() => {
          const subject = encodeURIComponent(
            `${t("carol.emailSubject")}${updated.product || t("carol.general")}`
          );
          ...
          addMessage({
            ...
            options: [
              {
                label: t("carol.sendEmailToCarol"),
                value: "send_email",
                icon: <Mail className="h-4 w-4" />,
              },
              {
                label: t("carol.startOver"),
                value: "restart",
                icon: <ArrowRight className="h-4 w-4" />,
              },
            ],
          ...
          submitInquiry.mutate({
            contactName: updated.product
              ? `${updated.product} inquiry`
              : "Website Visitor",
      ...
      if (value) addMessage({ role: "user", content: value });
        ...
        addMessage({ role: "user", content: t("carol.noAdditionalDetails") });
      setCurrentFormType(null);
    }
  };
    ...
    if (value === "send_email") {
      ...
      setTimeout(() => {
        addMessage({
          ...
          options: [
            {
              label: t("carol.startNewConversation"),
              value: "restart",
  ...
  const getPlaceholder = () => {
    switch (currentFormType) {
      case "product":
        return t("carol.productPlaceholder");
      case "quantity":
        return t("carol.quantityPlaceholder");
      case "contact":
        return t("carol.emailPlaceholder");
      case "details":
        return t("carol.detailsPlaceholder");
      default:
  ...
  const getFormLabel = () => {
    switch (currentFormType) {
      case "product":
        return t("carol.productType");
      case "quantity":
        return t("carol.quantityNeeded");
      case "contact":
        return t("carol.yourEmail");
      case "details":
        return t("carol.additionalDetails");
      default:
        return "";
    }
  };
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className={cn(
            "fixed bottom-24 right-6 z-50 flex flex-col rounded-2xl border shadow-2xl bg-card overflow-hidden",
            "w-[calc(100vw-3rem)] max-w-[380px] h-[520px] max-h-[calc(100vh-8rem)]"
          )}
        >
          <div className="flex items-center gap-3 px-4 py-3 bg-charcoal-dark text-white shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-white/60 truncate">
                {t("carol.dyPacksCustomerService")}
              </p>
            </div>
            <button
              onClick={handleToggle}
            ...
            <div className="flex flex-col space-y-4 p-4">
              {messages.map((message, index) => (
                <div key={index}>
                  ...
                  >
                    {message.role === "carol" && (
                      <div className="size-7 shrink-0 mt-0.5 rounded-full bg-gold/10 flex items-center justify-center">
                    ...
                    >
                      {message.options.map((option, optIndex) => (
                        <button
                          ...
                          className={cn(
                            "inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors bg-card hover:bg-gold/10 hover:border-gold-dark/30 text-foreground"
                          )}
                        >
                          {option.icon}
                          {option.label}
                      ...
                      {currentFormType === "details" ? (
                        <>
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              type="submit"
                              size="sm"
                              className="bg-gold text-charcoal-dark hover:bg-gold-dark text-xs h-8 ml-auto"
                            >
                        ...
                        <div className="flex gap-2">
                          <Input
                            ...
                            type={
                              currentFormType === "contact" ? "email" : "text"
                            }
                          />
                          <Button
                            type="submit"
                            size="icon"
                            className="bg-gold text-charcoal-dark hover:bg-gold-dark h-9 w-9 shrink-0"
